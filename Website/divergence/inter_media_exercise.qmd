...
```
```{r eval=FALSE}
#Create new column of left and right media
inter_media_wunifrac_long_tbl$media_L_R <-
    paste0(inter_media_wunifrac_long_tbl$media_L, "_",
            inter_media_wunifrac_long_tbl$media_R)
#Wilcox
pairwise.wilcox.test(inter_media_wunifrac_long_tbl$value,
                        inter_media_wunifrac_long_tbl$media_L_R)
```
...
In this case you should see there is a significant difference between the intra-group distances of all the different media types except for CVP compared to KBC and CVP compared to TSA.
...
There is a lot to unpackage when viewing the statistics of the media pair inter -group divergences.
Some notable points:
