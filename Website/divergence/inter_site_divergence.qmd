...
For our statistics we'll use a pairwise wilcox test to compare the inter-site divergences between the three sites.
...
We can be even finer with our statistics and compare all the site pairwise divergences.
To carry this out we'll create a column called `site_L_R` which contains the site pairing (e.g. LD_MD).
We'll use this column as our grouping.

To create the column `site_L_R` we have to do a bit more than usual.
For this column
...
inter_site_pairs_wunifrac_long_tbl <-
        inter_site_wunifrac_long_tbl |>
                ...
                dplyr::mutate(across(c(site_L,site_R), as.character)) |>
...
In this case all our p-values are 1, showing no significant difference.
Will more groupings and less data per grouping we are more likely to get higher p-values through stricter pp-value correction/adjustment and lower power.
