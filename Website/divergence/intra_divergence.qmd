...
<center>
![](images/apples_to_oranges_intra_group.png){style="width:500px; background: white; border-radius:5px"}
</center>
...
<center>
![](images/reason.png){style="width:150px; background: white; border-radius:5px"}
</center>
...
If a group has low intra-group distances this indicates the communities of the samples are quite similar.
Whereas, if a group has high intra-group distances this indicates the communities of the samples are quite dissimilar.
...
You are able to see intra-group divergence of this in ordination plots (PCoA, NMDS etc.) where group clusters being tighter than others indicates lower dissimilarity values.
For example the below (fictional) ordination shows tighter clustering in the gut microbiome of farmed fish compared to wild fish.
...
Divergence analysis directly uses the dissimilarity matrix values rather than ordination values that are calculated from the matrix.
...
## Groupings and pairs
<center>
![](images/intra_pair_hands.png){style="width:150px; background: white; border-radius:5px"}
</center>
...
__Note:__ We use the beta diversity distance (e.g. UniFrac) not the ordination distance (e.g. PCoA) for divergence analysis.
