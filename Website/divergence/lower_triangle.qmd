...
The next step is to convert the martix so all but the lower triangle values are `NA`s.
...
The diaganol values are set with the option `diag=`.
...
We can then subset out matrix with this function to assign the upper tirangle and diaganol values as `NA`.
...
With the above demonstrated let us create our lower triangle matrix.
For this we will create a new object for the lower triangle matrix so our edits won't change the inital `beta_df_mean` object which we will need later.
...
## Long data frame of lower triangel values

With our lower triangel matrix we can create a long data frame for future `ggplot2` plot production.

For this code we utlising [piping (`>`)](https://neof-workshops.github.io/Tidyverse/dplyr/pipes.html) with the below steps:
...
- Remove rows with NAs (i.e. the upper tirangle and diaganol values) with [`tidyr::drop_na()`](https://neof-workshops.github.io/Tidyverse/tidyr/drop_na.html)
