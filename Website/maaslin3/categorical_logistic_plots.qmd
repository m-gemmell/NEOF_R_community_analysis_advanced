...
This indicates that the [log-odds/logit](/maaslin3/logistic_model.qmd#log-oddslogit) of Streptococcus infantis_H being present in Rhizosphere Soil samples compared to Bulk Soil samples is higher by 4.4.
...
Below is the grid/tile plot created for the logistic modelling of *Quadrisphaera* for the sample site groupings. 
...
View the strongest associations (by q-value) with negative coefficient values.
These associations indicate the feature is less likely to be present in query samples compared to reference samples.
The further the value is from 0 the stronger the association.
...
For toher errors please see: [MaAsLin3 diagnostics](https://github.com/biobakery/Maaslin3#diagnostics)
...
Additionally, the abundance of the feature in the scalp samples it is found in are very low.
...
__Tip:__ The below filter function will retain rows where `error` is `NA`. Normally filtering won't work with `NA` values.
