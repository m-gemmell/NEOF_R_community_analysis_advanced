...
As we are plotting values from two different metrics we will split the figure into two separate plots.
`ggplot2::facet_wrap()` can be used for this tasked with `~metric` used to split the plot by the metrics.
We also specify `scales = "free"` so each of the plots has their own x and y scales.
...
::: {.callout-important}
We are using our wide data frame (`meta_alpha_mean_df`) rather than our long tibble (`basic_alpha_metrics_tbl`) for these stats.
:::
...
You'll see three p-value adjusted tables with most of the values being significant (<0.05).